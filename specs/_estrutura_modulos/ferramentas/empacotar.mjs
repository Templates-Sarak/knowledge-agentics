#!/usr/bin/env node
/**
 * empacotar.mjs — compila o backend TS (tsc), constroi o front onde existe (`web/`, vite), e
 ...
 *
 *   node ferramentas/empacotar.mjs                compila backend + front, em arvore (npm run build)
 *   node ferramentas/empacotar.mjs <destino>       idem, MAIS empacota o artefato backend em <destino>
 *   node ferramentas/empacotar.mjs --autoteste     prova interna (mesclarDependencias)
 *
 * SO TYPESCRIPT EMITE. Um modulo/raiz sem `tsconfig.build.json` e pulado em silencio informativo —
 ...
 *
 * ZERO SHELL, mesma tecnica de `verificar-commit.mjs` (ver o cabecalho de la para o porque —
 ...
 * nao importada: `entrypointDoPacote` já existe em `verificar-commit.mjs`, mas essa ferramenta está
 ...
 *   - `contrato/openapi.yaml`, `core/templates/*.html`, `database/**`: medido (grep no runtime nao-
 ...
 * `verificar-commit.mjs:entrypointDoPacote` — nao importada de nenhum dos dois (ver cabecalho).
 */
...
function empacotarBackend(raizProjeto, destino) {
  ...
  const destinoModulos = join(destino, 'modulos');
