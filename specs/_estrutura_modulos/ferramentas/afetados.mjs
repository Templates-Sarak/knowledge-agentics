#!/usr/bin/env node
/**
 * afetados.mjs — dado um conjunto de arquivos alterados, diz o que precisa ser VERIFICADO.
 * Lei dona: nenhuma — ferramenta, como `sincronizar-env.mjs` e `gerar-config-lint.mjs`. Não é
 * regra de gate, não tem id, não conta para as 73 regras com caso do catálogo.
 *
 *   node ferramentas/afetados.mjs <caminho...>          caminhos por argumento
 *   git diff --name-only | node ferramentas/afetados.mjs   caminhos por stdin (sem argumento)
 *   node ferramentas/afetados.mjs --desde <ref>          chama `git diff --name-only <ref>`
 *   node ferramentas/afetados.mjs ... --json             saida estruturada em vez de texto
 *   node ferramentas/afetados.mjs --autoteste            roda a prova interna (fixtures em memoria)
 *
 * ============================================================================================
 ...
 * ============================================================================================
 *
 * ARQUITETURA DO ARQUIVO — duas metades, como `gate/contexto.mjs` e `gate/spec.mjs` já separam:
 *
 *   - `montarGrafo` é a ÚNICA função que toca disco. Lê `modulo.json` de cada módulo (via
 *     `listarModulos`/`acharRaizProjeto`, REUSADOS de `gate/contexto.mjs` — a mesma definição de
...
/**
 * Monta o grafo do projeto a partir do disco: um nó por pasta de `modulos/` com manifesto (via
 * `listarModulos`, que JÁ inclui `_template` — "Lista as pastas de modulos/ que tem manifesto —
 * inclusive os moldes", contexto.mjs). Decisão sobre `_template` (armadilha do Passo 4): entra sem
 ...
 * uma mudança nele é, na prática, só ele mesmo — o mesmo isolamento que `validar.mjs --todos` já
 ...
 * `contexto.mjs:idDaPasta` usa, e a razão é dupla: (1) é o que aparece no CAMINHO do arquivo
 * alterado (`modulos/<pasta>/...`), que é o único dado que este script recebe; (2) `consome[].modulo`
 ...
 * tudo. `grafo` só é usado para confirmar que um `modulos/<id>` é um id que EXISTE hoje.
 *
 ...
 * Passo 1 sobre `ferramentas/`, `config/`, `.env`, `package.json`/`pyproject.toml` da raiz e config
 * de linter viram a MESMA linha de código: nenhum desses é `modulos/`, `adapters/`, `src/` ou
 ...
 * Medido (Passo 1): mudar o VERIFICADOR (`ferramentas/`) ou a POLÍTICA do projeto (config raiz,
...
function classificarCaminho(caminho, grafo) {
  ...
  if (primeiro === 'modulos') {
    if (segundo === undefined) {
      return { tipo: 'tudo', motivo: `caminho "${caminho}" aponta para modulos/ sem um modulo especifico` };
    }
    if (!grafo.modulos.has(segundo)) {
      ...
      return {
        tipo: 'tudo',
        motivo: `"${segundo}" nao existe em modulos/ hoje (apagado, renomeado, ou nome desconhecido) —`
  ...
  if (primeiro === 'adapters' || primeiro === 'src') {
    ...
    // código de NENHUM módulo; muda o sistema COMPOSTO — as regras de escopo `raiz`. Não é "tudo":
    // é uma resposta calculada, não um escape.
    return { tipo: 'fiacao' };
  }

  if (primeiro === 'packages') {
    // Medido (Passo 1): módulo importar `packages/portas` é OPCIONAL por projeto (o molde traz
    ...
    // `isolamento.mjs`, que este script não deveria duplicar. Sem essa máquina, não dá pra provar
  ...
  if (primeiro === 'ferramentas') {
    return { tipo: 'tudo', motivo: 'ferramentas/ e o proprio verificador — mudar o gate pode mudar o resultado de QUALQUER checagem' };
  }

  return {
    tipo: 'tudo',
    motivo: `caminho fora das areas conhecidas (modulos/, adapters/, src/, packages/, ferramentas/): "${caminho}"`,
  };
}
...
export function calcularAfetados(grafo, caminhosBrutos) {
  ...
  for (const caminho of caminhosBrutos) {
    if (caminho === null) {
      return { tudo: true, motivo: 'caminho fora da raiz do projeto (fora de modulos/, adapters/, src/, packages/, ferramentas/ e da propria raiz)' };
    }
 ...
 * `(raiz)` são sentinelas entre parênteses — nenhum id de módulo passa pela regra `manifesto`
 ...
 * dois campos (`tudo` e `modulos`) que nunca aparecem preenchidos ao mesmo tempo.
 */
function formatarTexto(resultado) {
  if (resultado.tudo) return '(tudo)\n';
  const linhas = resultado.raiz ? ['(raiz)', ...resultado.modulos] : [...resultado.modulos];
...
// ================================================================================================
// AUTOTESTE — a prova por máquina (lei 6: sem prova, não entra). `casos.mjs` prova REGRA do gate;
  ...
  return [
    {
      ...
      grafo: grafoDeTeste(),
      caminhos: ['modulos/a/core/dominio/index.ts'],
    ...
    },
    {
      ...
      grafo: grafoDeTeste(),
      caminhos: ['modulos/removido/core/dominio/index.ts'],
      esperado: { tudo: true },
    },
    {
    ...
    {
      nome: 'ferramentas/ (o proprio verificador) vira "tudo"',
      grafo: grafoDeTeste(),
      caminhos: ['ferramentas/gate/validar.mjs'],
      esperado: { tudo: true },
    },
    {
    ...
    },
    {
      ...
      grafo: grafoDeTeste(),
      caminhos: ['modulos/a/core/dominio/index.ts', 'modulos/solto/core/dominio/index.ts'],
...
function casosDeNormalizacao() {
  ...
  return [
    { bruto: 'modulos\\a\\core\\dominio\\index.ts', esperado: 'modulos/a/core/dominio/index.ts' },
    { bruto: 'modulos/a/core/dominio/index.ts', esperado: 'modulos/a/core/dominio/index.ts' },
    { bruto: 'C:\\tmp\\projeto\\modulos\\a\\core\\dominio\\index.ts', esperado: 'modulos/a/core/dominio/index.ts' },
...
function principal() {
  ...
  if (brutos === null) {
    process.stderr.write(
      'uso: node ferramentas/afetados.mjs <caminho...>\n'
      + '     node ferramentas/afetados.mjs --desde <ref>\n'
      + '     <algo que lista arquivos> | node ferramentas/afetados.mjs\n'
      + '     node ferramentas/afetados.mjs --autoteste\n',
    );
    return 1;
  }
...
// Só executa quando ESTE arquivo é o entrypoint — mesma guarda de `gerar-config-lint.mjs`: importar
