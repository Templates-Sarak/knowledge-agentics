#!/usr/bin/env node
/**
 * verificar-citacoes.mjs — prova que a campanha de rename (plan-3 Bloco AC) não deixa citação órfã
 ...
 *
 *   node testes/verificar-citacoes.mjs --antes      sanidade ANTES do rename: todo nome antigo do
 *                                                    inventário é citado no corpus, e a citação resolve
 *   node testes/verificar-citacoes.mjs --depois      prova DEPOIS do rename: zero ocorrência de nome
 *                                                    antigo em qualquer arquivo do corpus, e todo nome
 *                                                    novo citado resolve
 *   node testes/verificar-citacoes.mjs --autoteste   prova o núcleo com fixtures em memória
 *
 * INVENTÁRIO COMO ENTRADA (`inventario-renomeacao.json`, mesma pasta): a lista FECHADA de nomes que a
 ...
 *
 * Fora de `ferramentas/` DE PROPÓSITO — mesmo motivo e mesma pasta de `verificar-mapa.mjs` e
 * `autoteste-template.mjs` (D3, plan-2.md): ferramenta de quem MANTÉM a base, nunca de um projeto
 * gerado. `criar-projeto.mjs` copia `ferramentas/` inteiro para dentro de cada projeto — se este
 ...
 *
 * DIFERENÇA DELIBERADA para `verificar-mapa.mjs` (plan-2.1.md Bloco U): aquele confere o MAPA
 ...
 * de símbolo da Classe `simbolo`, usando a MESMA regra de cerca `"""`/`'''` que `gate/contexto.mjs`
 ...
 *
 * NÚCLEO × CASCA, precedente de `verificar-mapa.mjs`/`afetados.mjs`: toda função de extração e
...
const CAMINHO_INVENTARIO = join(AQUI, 'inventario-renomeacao.json');
 ...
 * `modulo` não casa dentro de `modulos` (o `s` seguinte é fronteira violada); `criar-modulo.mjs` casa
 * dentro de `ferramentas/criar-modulo.mjs` (a `/` anterior é fronteira válida). Fronteira é "não é letra,
 ...
 *   | `bindings/<b>/_template/`   | `modulos/<id>/`       |
 *
 * Dado um nome na forma do PROJETO, devolve a forma equivalente na BASE. `modulos/<id>/resto` perde
...
export function visaoDaBase(nome) {
  ...
  const semModulo = nome.match(/^modulos\/[^/]+\/?(.*)$/);
  if (semModulo !== null) return semModulo[1];
  return null;
}

/**
 * Mesma extração de comentário de `gate/contexto.mjs:extrairLinhasDeCodigo` — aqui separada nas DUAS
 ...
 * alcance de `ferramentas/gate/` por D3, e a cópia é da REGRA, não da fonte de verdade sobre o que é
 ...
 * pena tentar — a base primeiro, depois cada binding (`_template` antes de `raiz`). */
function raizesDeResolucao() {
  ...
  for (const binding of BINDINGS) {
    ...
    raizes.push(join(RAIZ_TEMPLATE, 'bindings', binding, 'raiz'));
  }
  return raizes;
}

const IGNORAR_NA_VARREDURA = new Set(['node_modules', '.git', '__pycache__', '.venv', 'dist', 'gerados']);
...
/** `bindings/**`, `ferramentas/**` (inclui `gate/regras/*.mjs` — mensagem de regra é código real, já
 * coberta aqui, sem precisar de extração separada), `testes/**`. */
function arquivosDeFonte() {
  return [
    ...arquivosSob(join(RAIZ_TEMPLATE, 'bindings'), EXT_FONTE),
    ...arquivosSob(join(RAIZ_TEMPLATE, 'ferramentas'), EXT_FONTE),
    ...arquivosSob(join(RAIZ_TEMPLATE, 'testes'), EXT_FONTE),
  ];
}
...
const SINAIS_DE_TEMPLATE = ['specs/_estrutura_modulos', 'criar-modulo.mjs', 'criar-projeto.mjs'];
...
function chavesDeManifesto() {
  const moduloSchema = JSON.parse(lerTexto(join(RAIZ_TEMPLATE, 'ferramentas', 'gate', 'schemas', 'modulo.schema.json')));
  const projetoSchema = JSON.parse(lerTexto(join(RAIZ_TEMPLATE, 'ferramentas', 'gate', 'schemas', 'projeto.schema.json')));
  return new Set([...chavesDoSchema(moduloSchema), ...chavesDoSchema(projetoSchema)]);
}

/** Todo identificador presente em CÓDIGO real (não comentário) de `bindings/**`, `ferramentas/**`,
 * `testes/**` e dos `scripts/**` das skills que citam o template — o universo contra o qual um nome de
...
/** Lê `inventario-renomeacao.json` — mesma disciplina de `config/conformidade.json`: começa vazio, e
...
function casosDeAutoteste() {
  const contexto = {
    ...
    indiceNomes: new Set(['validar.mjs']),
  ...
  return [
    ...
    { nome: 'ocorrenciasDoNome: acha nome com barra dentro de caminho maior (basename)', fn: () => (
      ocorrenciasDoNome('criar-modulo.mjs', 'rode `ferramentas/criar-modulo.mjs` agora').length === 1
    ) },
    ...
    { nome: 'resolveNome arquivo: por nome-base no indice', fn: () => resolveNome('arquivo', 'validar.mjs', contexto) === true },
    ...
    { nome: 'visaoDaBase remove <id> de modulos/<id>/resto', fn: () => visaoDaBase('modulos/catalogo/core/dominio') === 'core/dominio' },
...
function rodar(modo) {
  ...
  if (inventario.itens.length === 0) {
    process.stdout.write(
      'inventario-renomeacao.json vazio — nada a conferir. Estado correto antes do plan-3 Bloco AC\n'
...
function principal() {
  ...
  process.stderr.write('uso: node testes/verificar-citacoes.mjs --antes\n'
    + '     node testes/verificar-citacoes.mjs --depois\n'
    + '     node testes/verificar-citacoes.mjs --autoteste\n');
  return 1;
}
