...
export function encontrarRaizModulo(partida = process.cwd()) {
  let atual = partida;
...
function lerJson(raiz, relativo) {
  try {
    return JSON.parse(lerTexto(join(raiz, relativo)));
  } catch (causa) {
...
function aplicarSemSobrescrever(pares) {
 ...
 */
function resolverAmbiente(raizModulo) {
...
export function envObrigatoria(chave) {
 ...
 * `test` de proposito.
 */
export function conferirEnvRequerido(manifesto) {
...
export function carregarConfiguracao(raiz = encontrarRaizModulo()) {
  const manifesto = lerJson(raiz, 'modulo.json');
  ...
  return {
    raiz,
    manifesto,
    api: lerJson(raiz, 'config/api.json'),
    dominio: lerJson(raiz, 'config/dominio.json'),
    seguranca: lerJson(raiz, 'config/seguranca.json'),
    portas: lerJson(raiz, 'config/portas.json'),
    textos: lerJson(raiz, 'config/textos.json'),
  };
}
