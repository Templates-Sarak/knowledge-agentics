...
# `descobrir_modulos`) — `memory` ignora (nao precisa saber QUEM a chamou, por isso os `lambda`
...
def resolver_dependencias(modulo: dict[str, Any]) -> dict[str, Any]:
    ...
    """
    escolhas = _ler_json(modulo["pasta"] / "config" / "ports.json")
...
def montar_sistema(raiz: Path) -> RaizAsgi:
    ...
    for modulo in modulos:
        ...
        # `_importar_api_do_modulo` ja deixou o `core.ports` FRESCO deste modulo em `sys.modules`
...
def _casos_de_rotas_unicas() -> list[dict[str, Any]]:
    return [
        ...
        {"nome": "um so modulo", "modules": [_manifesto_de_teste("a", "/api/v1/a")], "espera_erro": False},
        {
            "nome": "rotas distintas",
            "modules": [_manifesto_de_teste("a", "/api/v1/a"), _manifesto_de_teste("b", "/api/v1/b")],
            "espera_erro": False,
        },
        {
            "nome": "rotas colidindo",
            "modules": [_manifesto_de_teste("a", "/api/v1/a"), _manifesto_de_teste("a2", "/api/v1/a")],
            "espera_erro": True,
        },
    ]
...
def _rodar_autoteste() -> int:
    ...
    for caso in _casos_de_rotas_unicas():
        lancou = False
        try:
            verificar_rotas_unicas(caso["modules"])
        except RuntimeError:
            lancou = True
