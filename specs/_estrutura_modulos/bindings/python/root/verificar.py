...
def _rodar_cobertura() -> int:
    ...
    modulos = _modulos()
    if not modulos:
        _escrever("  !    nenhum modulo em modules/ — nada para medir\n")
        return 0
...
def _rodar_delegado(rotulo: str, script: str) -> int:
    ...
    ok = _rodar(rotulo, ["node", f"tools/{script}"], None)
    return 0 if ok else 1
...
def main() -> int:
    ...
    if "--seguranca" in sys.argv:
        return _rodar_delegado("seguranca", "ci-security.mjs")
    if "--dependencias" in sys.argv:
        return _rodar_delegado("dependencias", "ci-dependencies.mjs")
    ...
    if not _modulos():
        _escrever("  !    nenhum modulo em modules/ — nada para testar\n")
