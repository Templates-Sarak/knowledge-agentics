/**
 * rules/structure.mjs — família "Estrutura" do catálogo (specs/arquitetura/04-regras.md §4.1).
 ...
 *
 * `manifesto-raiz` é de escopo `root` e mora aqui porque a FAMÍLIA é a mesma — o manifesto é o
...
const ENTRADAS_PERMITIDAS = new Set([
  ...
  // As DUAS travadas por maquina: `gate/tests/cases.mjs`, caso "schema-manifesto — CHAMARIZ" —
  ...
  // INALCANCAVEL: `gate/context.mjs:NAO_PERCORRER` (FORA de escopo) ja contem `'dist'`, e
  // `entradasRaiz` filtra tudo que esta em `NAO_PERCORRER` ANTES desta lista ser consultada
  // (unica excecao e `'generated'`, via `CONTEUDO_IGNORADO_MAS_ENTRADA`) — `dist` nunca chega a
  // `ctx.entradasRaiz`, entao uma entrada aqui nunca teria efeito nenhum. Lista normativa com
  // item inalcancavel e declaracao sem efeito (o mesmo defeito que `'generated'` teve nesta MESMA
 ...
 *
 * `generated/` se detecta pela ENTRADA da raiz, não por arquivo: o conteúdo dela é saída de máquina e
 * fica fora de `ctx.arquivos` de propósito (varrê-lo faria as regras de código julgar HTML gerado).
 * As outras duas são fonte escrita à mão, e ali o arquivo é a prova — pasta vazia não é `core/engine`.
 */
function temPastaDeArtefato(ctx, pasta) {
  if (pasta !== 'generated/') return temArquivoEm(ctx, pasta);
  return ctx.entradasRaiz.includes('generated');
}
...
export default [
  ...
  },
  {
    ...
    verificar(ctx) {
      ...
      // "contract/openapi.yaml ausente" e um defeito com duas mensagens — e a segunda nao acrescenta
  ...
  },
  {
    ...
    verificar(ctx) {
      ...
      if (!temArquivoEm(ctx, 'tests/contract/')) achados.push('tests/contract/ vazio ou ausente');
      return achados;
    },
  },
  {
  ...
  },
  {
     ...
     * couber (`tests/contract/`, `tests/domain/`) e o que se cobra é o NOME que espelha o gateway.
     */
    id: 'testes-gateway',
    nivel: 'erro',
    escopo: 'module',
