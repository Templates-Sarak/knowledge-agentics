...
num repositório com dez módulos. O achado sai sob o alvo `(root)`. Quais são elas está no catálogo
...
dois, prontos**, em `bindings/<binding>/root/.githooks/`. Cada um é duas linhas, idênticas nos três
bindings, e delegam para `tools/verify-commit.mjs` — que roda o gate só nos módulos
**afetados** pelo staged (`pre-commit`) ou pelo delta desde o upstream (`pre-push`), calculado por
`tools/affected.mjs`. Ver `specs/arquitetura/03-operacao.md` §7.1 para a fiação completa das três
...
.githooks/pre-push` uma vez (`create-project.mjs` já imprime isso nos "próximos passos").
...
ler o `contract/openapi.yaml` de ANTES, via git, e o gate não roda git de propósito). Quem responde é
...
2. Acrescente o objeto ao arquivo da família em `rules/`:
...
5. **Leitura de `contract/openapi.yaml` já existe** em `tools/gate/spec.mjs` (`specDe`,
...
6. **Regra que julga CÓDIGO lê `textoDeCodigo`** (`tools/gate/text.mjs`), nunca
