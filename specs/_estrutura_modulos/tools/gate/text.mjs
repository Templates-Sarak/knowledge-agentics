/**
 * text.mjs — o texto de um arquivo **sem comentário nem docstring**. Lei dona:
 * specs/arquitetura/04-regras.md §7.2 ("Leitura de código: comentário não é código").
 *
 * Vive fora de `rules/` pelo mesmo argumento que `spec.mjs`, e aqui ele é mais forte: CINCO das
 * seis famílias precisam dela. Enquanto morava em `rules/isolation.mjs`, a última a chegar —
 * `operation.mjs`, que já é importada por `isolation.mjs` (`SQL_FONTE`) — fechou um CICLO entre duas
 ...
 * `linhasCodigo`, montado uma vez por `context.mjs` em vez de recalculado a cada regra. Isso é
 * conserto em `context.mjs`, que está fora do alcance deste bloco — registrado aqui para não ser
 * redescoberto.
 */
