...
| `conformidade-declarada` | verifica **forma**, e só ela — o mesmo par existe+schema de `verificacao-declarada` (linha acima), com `tools/gate/schemas/conformidade.schema.json` fechando `additionalProperties` e as chaves obrigatórias de cada entrada de `excecoes`/`excecoesCve`. Três limites, nenhum deles de forma. **(1) Não resolve `decisao` contra um ADR de verdade** — isso é da própria lista de exceções (§6): `carregarExcecoes` reprova a exceção inteira se `decisao` não apontar para um `## ADR-NNN` real em `specs/adr/*.md`, e é trabalho **daquela** verificação, não desta regra. **(2) Não valida que `regra` exista no catálogo** — uma exceção com `"regra": "regra-que-nao-existe"` passa o schema (é string não vazia) e nunca perdoa achado nenhum, porque `aplicarExcecoes` só cruza `modulo`+`regra` contra os achados reais do gate; é ***fail-closed*** (a regra continua acusando, ninguém se machuca), mas **silencioso** — nada diz que a exceção é inútil. **(3) Não valida que `modulo` exista** — mesmo raciocínio, mesma consequência. As duas últimas exigiriam cruzar a lista de exceções com o catálogo de regras e com os módulos reais do projeto, e uma regra de escopo `root` só recebe `ctx.projeto` (§4.1) — enxergar os módulos pediria escopo `global`, mudança de verdade, não conserto |
