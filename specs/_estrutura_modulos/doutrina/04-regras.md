...
não se leu manda o autor apagar o que está certo. Por isso `contrato/openapi.yaml` ilegível **ou rota
...
maiúscula (TS/JS) ou `para_` (Python) — nunca uma REFERÊNCIA (`registros.map(paraContrato)` não é
