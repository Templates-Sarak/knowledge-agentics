...
| `env-raiz-declarado` | lê `adapters/`, `src/` e `packages/`, e **só**. `ferramentas/` fica fora de propósito — é o gate, que o template instala e ninguém edita —, e `modulos/` já tem as 57 regras dele. Consequência assumida: chave `RAIZ_*` consumida **fora** dessas três pastas (um `docker-compose.yml`, um script de operação) é acusada como declarada-e-sem-leitor. Não é falso positivo por acidente: env da raiz se lê na composição, que é onde a fiação mora. Como `env-declarado`, ela varre o **conteúdo** do arquivo e não só a linha de código, então a chave citada em comentário conta como usada — falso negativo na direção estrita, e o que impede o comentário de virar acusação na outra. O prefixo `RAIZ_` é reservado por convenção (§3.1) e não por mecanismo: um módulo cujo `id` fosse literalmente `raiz` produziria chaves `RAIZ_*` indistinguíveis das da raiz. Limite conhecido, e o único caso em que as duas convenções colidem |
...
| **Leitura de código: comentário não é código** | Toda regra que julga CÓDIGO lê `textoDeCodigo` — o arquivo sem comentário nem docstring —, e **nunca** o texto cru. É o que impede a própria lei escrita num comentário de virar violação dela mesma: um bloco documentando *"como NÃO fazer: `import { X } from '@acme/fin'`"* acusava `import-lateral`, `import-adapter` e `sdk-fornecedor` de uma vez, sobre código correto. Vale para as oito regras que leem import (`import-lateral`, `import-adapter`, `sdk-fornecedor`, `ui-kit`, `portas-pura`, `adapter-isolado`, `composicao-descoberta`) mais `gateway-http`, `env-declarado`, `env-raiz-declarado`, `env-fora-do-carregador` e `tabela-alheia`. **O recorte não é total, e o resto fica aqui:** `linhasCodigo` remove comentário e docstring, **não string** — `const exemplo = "import x from '@acme/y'"` continua sendo visto, porque é código de verdade, e separar literal de instrução exigiria AST. Falso positivo muito mais raro, e declarado em vez de prometido resolvido |
| Quem lê o texto CRU, e deve | `migrations` procura `-- rollback`, que **é** um comentário SQL — ali o comentário é o dado, não ruído. Idem `lerParesEnv` sobre `.env`/`.env.example`, todos os leitores de `openapi.yaml` e o `juntarSql` de `rls`/`tabela-declarada` (o extrator de linhas não remove `--`, então em SQL ele não ajudaria). **Duas regras leem cru e erram para o SILÊNCIO**, o que é tolerado por ser a direção conservadora: `config-morta` conta uma chave citada em comentário como "lida" (falso negativo num `aviso`), e a cláusula de origem do `rota-publica-autenticada` aceita `rotasPublicas` citado em comentário como "a `api/` lê o manifesto" (falso negativo). Corrigi-las as tornaria mais estritas, o que é mudança de comportamento e não conserto de falso positivo |
| `contrato-sincronizado`, `projecao-contrato`, `payload-camelcase`, `sensivel-em-saida` | **ainda leem o texto cru, e têm o falso positivo do comentário**: rota comentada (`// router.get('/nunca-existiu', …)`) vira rota fantasma no código, e mapeador comentado vira projeção fantasma. Ficou declarado em vez de consertado porque os dois extratores de `contrato.mjs` são leitores POSICIONAIS — um casa um padrão de várias partes sobre o arquivo inteiro, o outro delimita a projeção **balanceando chaves** —, e trocar o texto de entrada muda deslocamento e fronteira de região. É conserto com raio de quatro regras e casos próprios, não uma linha |
