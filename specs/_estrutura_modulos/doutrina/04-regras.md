...
|---|---|---|---|
| `manifesto` | erro | `modulo.json` existe e é JSON válido; campos obrigatórios presentes; `id` igual ao nome da pasta; `rotaBase` igual a `/api/v1/<id>`; `binding` no vocabulário. `papel` **não** entra — é enum, e quem acusa enum é `schema-manifesto` (§7.2, R.2) | módulo |
| `schema-manifesto` | erro | `modulo.json` conforma ao JSON Schema (`tools/gate/schemas/modulo.schema.json`): tipo, formato e vocabulário de cada campo, **e campo não previsto reprova** | módulo |
| `estrutura` | erro | `api/`, `tests/`, `core/domain/`, `core/ports/` e `README.md` presentes; os cinco `config/*.json` presentes; e o arquivo de manifesto/tipos que o `binding` declarado exige (`tsconfig.json` **e** `package.json` em TypeScript, `package.json` em JavaScript, `pyproject.toml` em Python) — o conjunto obrigatório do molde que não tinha dono (plan-2.md Bloco M). `contract/openapi.yaml` é do `contract`; `modulo.json` é do `manifesto`; `core/gateways/` fica de fora, de propósito — módulo sem `consome` legitimamente não tem gateway (§7.2) | módulo |
...
| `artefato-declarado` | erro | o `geraArtefato` do manifesto e a árvore concordam, **nos dois sentidos**: `true` exige `core/engine/`, `core/templates/` e `generated/`; `false` proíbe as três. `database/` **não** entra — quem declara banco é `dados.tabelas` (§7.2) | módulo |
| `tests` | erro | `tests/domain/` não-vazio; `tests/contract/` não-vazio em módulo com rota | módulo |
| `testes-web` | erro | módulo que declara `rotaWeb` tem `tests/web/` não-vazio — a terceira camada do [[03-operacao]] §5. **Condicional**, com a mesma guarda de `web-declarado`: `rotaWeb: null` silencia, e `create-module.mjs --sem-web` zera os dois de uma vez. Verifica presença, **não** os três estados que o §5 pede (§7.2) | módulo |
| `testes-gateway` | erro | todo gateway REAL de `core/gateways/` (barril `index`/`__init__` excluído) tem teste cujo nome o espelha (`<gateway>.test.*`, `test_<gateway>.*`). Fecha o triângulo `gateway ⟷ consome ⟷ teste`: `gateway-declarado` liga o arquivo ao `consome`, `consome-contrato` liga o `consome` ao contrato do dono, e esta liga o gateway ao teste. **Não** é `tests` com outro nome — aquela cobra a PASTA, esta cobra um teste POR gateway. A pasta do teste não é imposta (§7.2) | módulo |
| `manifesto-raiz` | erro | a raiz do projeto tem `projeto.json`, é JSON válido e conforma ao JSON Schema (`tools/gate/schemas/projeto.schema.json`), **campo não previsto reprova**. **Um id só** — desde o Bloco R.2 (plan-2.md), `manifesto` do módulo também é, pela mesma razão: tudo que o manifesto afirma além das cláusulas relacionais (`id`, `rotaBase`) é forma, e a raiz não tem nem essas. Módulo solto, fora de projeto, silencia (§7.2) | raiz |
...
| `consome-contrato` | erro | toda entrada de `consome` aponta para um módulo que existe, e o `contract/openapi.yaml` dele declara aquele caminho **e** aquele método. Dono sem spec é achado, não silêncio. Reportado no **consumidor** | global |
...
| `composicao-descoberta` | erro | nenhum arquivo de `src/` importa de `modules/`. **Import é dependência; leitura de arquivo é descoberta** — a composição alcança todos os módulos lendo o `modulo.json` de cada pasta, e isso é a doutrina funcionando ([[00-arquitetura]] §3.4). Importar `adapters/` daqui é o ofício da fiação e não é acusado (§7.2) | raiz |
...
| `schema-config` | erro | cada `config/*.json` conforma ao seu JSON Schema em `tools/gate/schemas/`. `api`, `seguranca` e `portas` têm forma fechada (campo não previsto reprova); `domain` e `textos` são livres por definição | módulo |
| `porta-declarada` | erro | as chaves de `config/ports.json` e as entradas de `modulo.json:portas` coincidem **nos dois sentidos** — as duas metades de uma declaração: o manifesto diz o que o módulo EXIGE, a config diz QUEM preenche. Declarada e não configurada **derruba o boot**; configurada e não declarada é config morta. O JSON Schema não podia cobrar isto e afirmava que cobrava: `additionalProperties: false` fecha o vocabulário, não amarra ao manifesto (§7.2) | módulo |
...
| `rota-publica-autenticada` | erro | a isenção de autenticação é a do manifesto e nada mais: a `api/` **lê** `modulo.json:rotasPublicas`, nenhum arquivo dela escreve rota `"MÉTODO /caminho"` como literal, e **toda** entrada de `rotasPublicas` existe no `contract/openapi.yaml` com **aquele método** — entrada com typo não isenta nada e ninguém percebe. Verifica a origem e a realidade da lista, **não** a ordem da cadeia nem a rejeição em runtime (§7.2) | módulo |
| `permissao-literal` | erro | nenhum argumento de `requirePermission` na `api/` é literal — a permissão em vigor vem de `modulo.json:permissoes`, nunca do código, senão o manifesto declara uma coisa e a rota exige outra. É a simétrica da cláusula de origem do `rota-publica-autenticada`. A metade inversa (*toda permissão declarada é exigida em alguma rota*) **não** existe, e não por descuido: ela é incompatível com esta (§7.1) | módulo |
...
Achado de escopo `root` também se excetua por essa lista, e o `modulo` dele é o literal `"(root)"` — o mesmo
...
**Extração de texto de spec e de mapeador** — `contract`, `contrato-sincronizado`, `rota-nomenclatura`,
...
que `resumo-exportado` lê) tem **um dono só**, a regra `contract`, e as **demais** silenciam nesse caso —
...
> inteira, vivendo dentro de uma célula de tabela de ~600 palavras — o mais difícil de achar e de citar
> por âncora. O conteúdo não mudou, só o endereço.

**O extrator (plan-2.md N.2) tem DUAS âncoras, e as duas têm limite próprio.** As duas se apoiam num
ÚNICO reconhecedor de "sítio de definição" (plan-2.md N.2.2 — antes eram dois, um por âncora, e
"quase concordar" já rendeu três falsos positivos, um por forma de sintaxe nova). O reconhecedor casa
...
forma, método de objeto/classe (`nome(...)`), propriedade-arrow (`nome: (...) => ({…})`) e atribuição
de módulo em Python (`nome = lambda r: {...}`).
...
sítio de topo, recuo de método fecha em método vizinho da mesma classe (plan-2.md N.2.1), e recuo de
propriedade fecha em propriedade vizinha do mesmo objeto (plan-2.md N.2.2). O guardião é o RECUO, não
...
`paraGama: (r) => (...)` num objeto literal, ou `nome = lambda r: {...}` depois de outro no nível do
...
foi montar cada app sob a própria `rotaBase` na raiz (nunca em `"/"`), com `createApp` (TS/JS) ciente do
...
*.ts`, sem `src/composicao.ts`, sem `tools/` — reprodução colada no relatório desta tarefa, não
